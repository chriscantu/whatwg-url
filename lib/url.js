...
URLStateMachine.prototype["parse" + STATES.NO_SCHEME] = function parseScheme(c, c_str) {
  //jshint unused:false
  if (this.base === null || (relativeSchemas[this.base.scheme] === undefined && c_str !== "#")) {
    throw new TypeError("Invalid URL");
  } else if (relativeSchemas[this.base.scheme] === undefined && c_str === "#") {
    this.url.scheme = this.base.scheme;
    this.url.scheme_data = this.base.scheme_data;
...
URLStateMachine.prototype["parse" + STATES.RELATIVE_SLASH] = function parseScheme(c, c_str) {
  if (c_str === "/" || c_str === "\\") {
    ...
    if (this.url.scheme !== "file") {
      ...
      this.url.host = this.base.host;
      this.url.port = this.base.port;
    } else if (this.base.path.length && isASCIIAlpha(this.base.path[0].codePointAt(0)) && this.base.path[0][1] === ":") {
  ...
  if (c_str === "@") {
    ...
    for (let pointer = 0; pointer < len; ++pointer) {
      ...
      if (this.url.password !== null) {
        this.url.password += defaultEncode(c);
      } else {
        this.url.username += defaultEncode(c);
      }
    }
    this.buffer = "";
